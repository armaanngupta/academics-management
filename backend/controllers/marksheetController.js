...
exports.addMarksheet = async (req, res) => {
  try {
    const { studentName, rollNumber, academicYear, session, subject, degree, university, remarks, description } = req.body;
    ...
    const existingMarksheet = await Marksheet.findOne({ rollNumber });
    if (existingMarksheet) {
      return res.status(400).json({ message: 'Marksheet for this roll number already exists' });
    }
  ...
  try {
    ...
    unissuedMarksheets.forEach((item) => {
      ...
      if (ageDays <= 7) {
        ...
        ageBuckets['8-30 days'] += 1;
      } else if (ageDays <= 90) {
        ageBuckets['31-90 days'] += 1;
      } else {
        ageBuckets['90+ days'] += 1;
      }
    });
    ...
    const summary = {
      ...
      byAcademicYear: Object.entries(byAcademicYear)
        .map(([name, value]) => ({ name, value }))
...
// Search marksheets by name or roll number
...
exports.updateMarksheet = async (req, res) => {
  try {
    const { id } = req.params;
    const { studentName, rollNumber, academicYear, session, subject, degree, university, remarks, description, issued } = req.body;
    const remarkValue = remarks ?? description;
    ...
    if (rollNumber) marksheet.rollNumber = rollNumber;
...
exports.toggleIssuedStatus = async (req, res) => {
  try {
    ...
    res.json({
      message: 'Marksheet status updated successfully',
      marksheet,
    });
  } catch (error) {
